...
const parseDigits = (val: string, reg: RegExp) => val.replace(reg, '');

Yup.addMethod(Yup.string, 'cpf', function(message: string) {
  return this.test('cpf', message, function(val?: string) {
    if (!val) return true;
    if (!/[0-9]{3}\.[0-9]{3}\.[0-9]{3}-[0-9]{2}/g.test(val)) return false;
...
Yup.addMethod(Yup.string, 'cnpj', function(message: string) {
  return this.test('cnpj', message, function(val?: string) {
    if (!val) return true;
    if (!/[0-9]{2}\.[0-9]{3}\.[0-9]{3}\/[0-9]{4}-[0-9]{2}/g.test(val)) return false;
